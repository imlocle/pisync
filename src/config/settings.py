...
class Settings:
    ...
    def _load_config(config_path: Path) -> dict:
        try:
            ...
            logger.log_signal.emit(
                f"Error decoding {config_path}: {e}, using empty defaults."
            )
            return {}
        except Exception as e:
            logger.log_signal.emit(
                f"Error loading {config_path}: {e}, using empty defaults."
            )
            return {}
    ...
    def save_config(self, config_data: dict):
        ...
        try:
            ...
            logger.log_signal.emit(f"Successfully saved config to {config_path}")
        except Exception as e:
            logger.log_signal.emit(f"Error saving config to {config_path}: {e}")
